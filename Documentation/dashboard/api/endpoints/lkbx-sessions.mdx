...
<ResponseField name="name" type="string">
  Session name (from the lock settings template).
</ResponseField>
...
<ParamField body="lockSettingsId" type="number">
  The ID of a lock settings template to use. If omitted, default settings are
...
<ParamField body="keyholderIds" type="number[]">
  ...
  self-lock. Only used with `"lock"` action. All IDs must be users you have access to.
</ParamField>

<ParamField body="targetUserId" type="number">
  The user to lock/unlock. Defaults to the authenticated user. Must be a user you have access to.
</ParamField>

<ParamField body="isTestLock" type="boolean">
  Override the test lock setting from the lock settings template. When `true`, the lock owner can unlock without a keyholder. Only used with `"lock"` action.
</ParamField>
