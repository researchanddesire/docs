...
<div className="not-prose">`GET /api/v1/dtt/settings`</div>
...
<RequestExample>
  ```bash cURL curl -H "Authorization: Bearer YOUR_TOKEN" \
  "https://dashboard.researchanddesire.com/api/v1/dtt/settings?limit=50&offset=0" ```
</RequestExample>
...
<div className="not-prose">`GET /api/v1/dtt/settings/{id}`</div>
...
<div className="not-prose">`PATCH /api/v1/dtt/settings/{id}`</div>
...
| `404` | Settings not found or not owned by you |
