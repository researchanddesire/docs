...
If you experience unusual behavior while on alpha firmware—such as the dashboard showing your device as offline when it appears connected, or lock sessions stuck in a pending state—try these steps:

1. **Hard reboot with resync**: Press and hold both buttons (Enter/Lock and Back) for 5 seconds to force a resync with the dashboard
...
3. **Reflash to stable firmware**: If issues persist, use the web flasher to return to stable firmware as described above. After flashing, you may need to remove the device from your account (Settings → Devices) and re-pair it

<Card title="How to Flash Your Lockbox" icon="rotate" href="./flashing">
  Step-by-step instructions for reflashing firmware.
</Card>
