...
<Accordion title="How do I connect my Lockbox to WiFi?">
  ...
  <Card title="WiFi Troubleshooting" icon="wifi" href="/lkbx/quick-start/wifi-setup">
...
<Accordion title="My Lockbox keeps restarting on its own (power cycling)">
  If your Lockbox continuously reboots without giving you a chance to press any buttons, this is usually a firmware issue that can be resolved with the web flasher.

  **To fix power cycling:**

  1. Connect your Lockbox to your computer via USB-C (use a cable that supports data transfer, not just charging)
  2. Open [the web flasher](https://dashboard.researchanddesire.com/app/tools/web-flasher) in Chrome or another Chromium-based browser
  3. Click **Connect** and select your device from the list
  4. **Do not** check the "Erase device" box to preserve your settings
  5. Complete the flash process

  Once the flash completes, your device should restart normally and stop the power cycling behavior.
...
<Accordion title="My lock randomly unlocks or doesn't sync properly">
  If your Lockbox unlocks unexpectedly (for example, after a set number of hours when it shouldn't), or if lock commands from the dashboard aren't being applied correctly, try a hard reboot to resync the device with your lock session.

  **To hard reboot and resync:**

  1. Ensure your device is charged
  2. Press and hold **both buttons** (Enter/Lock and Back) simultaneously for 5 seconds
  3. The device will restart and resync with the dashboard

  This forces the device to re-establish its connection and pull the latest lock state from the server.
...
<Accordion title="Dashboard shows offline but my device shows connected">
  If the dashboard displays your Lockbox as "Offline" but your device shows a green connection indicator, your device and the dashboard may be out of sync.

  **Try these steps in order:**

  1. **Wait up to 60 seconds** — The dashboard can take up to a minute to reflect the device's actual status
  2. **Hard reboot with resync** — Press and hold both buttons (Enter/Lock and Back) for 5 seconds to force a full resync
