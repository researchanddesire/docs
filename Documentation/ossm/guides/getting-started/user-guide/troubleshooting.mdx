...
**Solution:**
Follow the [belt tensioning steps](#clicking-noise-during-homing) above to verify and adjust your belt tension. Even small amounts of slack can significantly affect performance.
</Accordion>
...
**Solution:**
1. If you have a multimeter, measure the voltage at the power input terminals while the OSSM is operating—it should remain between 23V and 25V under load
2. Try a different power supply if available
3. Ensure you're using a supply rated for at least 5A (10A recommended)
...
If selecting certain menu options—such as **Get Help**, **WiFi Setup**, or **Update**—causes the display to go blank or show no response, these features may not yet be available on your firmware version.

<AccordionGroup>
<Accordion title="Check firmware version">
**Current status:**

Some menu options visible on the wired remote are placeholders for features still in development. As of early 2026, the following may display but not function on all firmware versions:

- **Get Help** — Intended to display a QR code linking to documentation
- **WiFi Setup** — Intended to configure wireless network connectivity
- **Update** — Intended to check for over-the-air firmware updates

<Note>
If the display goes blank when selecting these options, wait a few seconds and then press the encoder to return to the menu. This is expected behavior for unreleased features—your remote is not malfunctioning.
</Note>
</Accordion>

<Accordion title="When will these features work?">
Feature availability depends on your firmware version:

- **WiFi Setup and Update** — These require the OSSM to connect to a WiFi network. If WiFi configuration hasn't been completed via the [Web BLE Controller](/ossm/guides/getting-started/wifi-setup#option-1-web-ble-controller), the menu options may not function as expected.
- **Get Help** — This feature is being rolled out in upcoming firmware releases.
...
<Accordion title="Resolution">
If you need WiFi connectivity or firmware updates:
...
These browser-based tools provide the same functionality without relying on the wired remote's menu options.
</Accordion>
</AccordionGroup>
