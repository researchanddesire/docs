...
From PlatformIO Home, select "Open Project" and choose the OSSM folder that contains <code>platformio.ini</code> (lowercase).
...
<Check>
The Explorer should show <code>platformio.ini</code>, a <code>src/</code> folder, and a <code>lib/</code> folder.
</Check>
</Step>

<Step title="Select the correct environment (if applicable)">
If the project defines multiple environments in <code>platformio.ini</code> (e.g., different boards or build options), use the environment selector in the VS Code Status Bar (usually labeled with the active environment) to pick the one that matches your board.
...
Open <code>src/main.cpp</code> to review the firmware source.
...
<Check>
A successful build ends with <code>SUCCESS</code> in the terminal. A successful upload shows <code>Hash of data verified</code> or a similar confirmation from the ESP32 uploader.
</Check>
</Step>
</Steps>
...
- Monitor serial output: Click the plug icon (Monitor) in the Status Bar, or run <code>PlatformIO: Monitor</code> from the Command Palette.
- Clean build: Run <code>PlatformIO: Clean</code> to remove compiled artifacts before rebuilding.
...
On Windows, the board appears as <code>COMx</code>. On macOS/Linux, it appears under <code>/dev/tty.*</code> or <code>/dev/cu.*</code>.
...
<Note>
The reference OSSM board target is <strong>Espressif ESP32 Dev Module</strong>. Ensure your <code>platformio.ini</code> environment uses the correct <code>board</code> setting for ESP32.
</Note>
</Accordion>

<Accordion title="Build fails due to library or platform updates">
An update to a platform or library may introduce breaking changes. Pin versions in <code>platformio.ini</code> to restore a known-good configuration.
...
- Check <code>monitor_speed</code> in <code>platformio.ini</code> (e.g., <code>115200</code>)
